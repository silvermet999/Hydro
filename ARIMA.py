...
class ARIMA:
    ...
    def plot_differance(self, data_set, d, column_name):
        ...
        plot(fig, auto_open=False, filename=f"difference_{column_name}.html")
        return differance
    ...
    def calculate_AR_weights(self):
        ...
        self.AR_weights = np.dot(
            np.dot(np.linalg.pinv(np.dot(normal_matrix_tanspose, normal_matrix)), normal_matrix_tanspose), self.Z)
    ...
    def calculate_AR_MA_weights(self):
        ...
        normal_matrix_tanspose = normal_matrix.transpose()
        self.weights = np.dot(
            np.dot(np.linalg.pinv(np.dot(normal_matrix_tanspose, normal_matrix)), normal_matrix_tanspose), differance)
    ...
    def plot_autocorrelation(self, data_set, lag, column_name):
        ...
        fig = dict(data=traces, layout=layout)
        plot(fig, auto_open=False, filename=f"autocorrelation_{column_name}.html")

    def plot_partial_autocorrelation(self, data_set, lag, column_name):
        pac = np.zeros(lag)
        pac_x = np.arange(lag)
        ...
        fig = dict(data=traces, layout=layout)
        plot(fig, auto_open=False, filename=f"partial_autocorrelation_{column_name}.html")

    def plot_residuals(self, data_set, prediction, column_name):
        x = np.arange(len(data_set))
    ...
    def plot_data(self, data_set, time, column_name):
        ...
        fig = dict(data=traces, layout=layout)
        plot(fig, auto_open=False, filename=f"data_raw_{column_name}.html")
        ...
        fig = dict(data=traces, layout=layout)
        plot(fig, auto_open=False, filename=f"data_norm_{column_name}.html")
    ...
    def plot_result(self, time, data, prediction, column_name):
        ...
        fig = dict(data=traces, layout=layout)
        plot(fig, auto_open=False, filename=f"results_{column_name}.html")
...
def run_arima_for_column(column_name):
    print("=" * 60)
    print("Running ARIMA pipeline for column: " + str(column_name))
    print("=" * 60)
    ...
    training_set_time = np.array(time_col[0:training_end])
    ...
    validation_set_time = np.array(time_col[validation_start:validation_end])
...
for col in feature_columns:
    try:
        results[col] = run_arima_for_column(col)
    except Exception as e:
        print("Skipping column " + str(col) + " due to error: " + str(e))